...
def return_400_if_object_found(model1, model2, number_of_barcodes, **filters):
    ...
    obj1 = model2.objects.filter(b_type=obj.b_type,gen_slot='Y').last()
    print(obj.b_type)
    # If no object is found in model2, handle the error (return None or an appropriate response)
    if not obj1:
        return None
    # Calculate the number of barcodes
    no_of_barcode_calculation = (obj1.last_barcode//10 - obj1.start_barcode//10)+1
    print(no_of_barcode_calculation)
    ...
    if no_of_barcode_calculation == number_of_barcodes:
        print(number_of_barcodes)
        print(type(number_of_barcodes))
