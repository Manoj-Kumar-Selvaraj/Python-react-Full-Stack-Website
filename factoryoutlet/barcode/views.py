...
from barcode_print.serializers import BarcodeTSerializer
...
class GenerateBarcodeView(APIView):
    ...
    # print(user)
    # user_id=EmployeeT.objects.filter(eid=user)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            Employee_instance=EmployeeT.objects.get(eid=user)
            ...
            if valid_type_instance:
                last_barcode = valid_type_instance.last_barcode+1
                ...
                barcode_duplicate=barcode_print_init(request,barcodes,number_of_barcodes,pname,psize,ptype,seller,pamount)
                if barcode_duplicate:
                    return Response({"error":"Duplicate Barcode Generation Alert, If you wish to generate again please change the slot to N"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def new_barcode(self, prefix, number, last_barcode):
        ...
        return barcodes
class BarcodeTSlotView(APIView):
    authentication_classes = [CustomTokenAuthentication]
    permission_classes = [IsAuthenticated]
    def get(self, request):
         Barcode_Log=BarcodeT.objects.all()
         serializer=BarcodeTSerializer(Barcode_log,many=True)
